/* 
 * patternlab-node - v0.1.0 - 2014-05-07 
 * 
...
module.exports = function(grunt) {
  ...
  grunt.registerTask('patternlab', 'create design systems with atomic design', function(arg) {
    ...
    var patternlab = {};
    patternlab.data = grunt.file.readJSON(config.patterns.data + 'data.json');
    patternlab.listitems = grunt.file.readJSON(config.patterns.data + 'listitems.json');
    patternlab.header = grunt.file.read(config.patterns.patternlab + 'pattern-header-footer/header.html');
    patternlab.footer = grunt.file.read(config.patterns.patternlab + 'pattern-header-footer/footer.html');
    patternlab.patterns = [];
    ...
    grunt.file.recurse(config.patterns.source, function(abspath, rootdir, subdir, filename){
      ...
      if(patternIndex === -1){
        ...
        if(grunt.util._.str.include(filename, 'json')){
          ...
          var currentPatternHeader = mustache.render(patternlab.header, currentPattern.data, patternlab.partials);

          grunt.file.write(config.patterns.public + flatPatternPath, currentPatternHeader + currentPattern.patternPartial + currentPatternFooter);
    ...
    var styleguideTemplate = grunt.file.read(config.patterns.patternlab + 'styleguide.mustache');
    ...
    var patternlabSiteTemplate = grunt.file.read(config.patterns.patternlab + 'index.mustache');
    ...
    //patternNav
    var patternNavTemplate = grunt.file.read(config.patterns.patternlab + 'partials/patternNav.mustache');
    var patternNavPartialHtml = mustache.render(patternNavTemplate, patternlab);

    //ishControls
    var ishControlsTemplate = grunt.file.read(config.patterns.patternlab + 'partials/ishControls.mustache');
    var ishControlsPartialHtml = mustache.render(ishControlsTemplate);

    //patternPaths
    var patternPathsTemplate = grunt.file.read(config.patterns.patternlab + 'partials/patternPaths.mustache');
    var patternPathsPartialHtml = mustache.render(patternPathsTemplate, {'patternPaths': JSON.stringify(patternlab.patternPaths)});

    //viewAllPaths
    var viewAllPathsTemplate = grunt.file.read(config.patterns.patternlab + 'partials/viewAllPaths.mustache');
    var viewAllPathersPartialHtml = mustache.render(viewAllPathsTemplate, {'viewallpaths': JSON.stringify(patternlab.viewAllPaths)});

    //websockets
    var websocketsTemplate = grunt.file.read(config.patterns.patternlab + 'partials/websockets.mustache');
